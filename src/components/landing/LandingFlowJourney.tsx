...
interface LandingFlowJourneyProps {
  content: Pick<
    LandingProcessSectionContent,
    "journeyDescription" | "journeyLabel" | "journeyTitle" | "outcomes" | "stages"
  >;
}
...
export function LandingFlowJourney({ content }: LandingFlowJourneyProps) {
  return (
    <Card
      className="landing-flow-spotlight overflow-hidden rounded-[1.75rem] border-neutral-border bg-[radial-gradient(circle_at_top_left,rgba(103,232,249,0.16),transparent_26%),radial-gradient(circle_at_bottom_right,rgba(250,204,21,0.18),transparent_30%),linear-gradient(180deg,#0f172a_0%,#111827_100%)] text-white shadow-hero-soft sm:rounded-[2rem]"
      noPadding
    >
      <CardContent className="p-5 sm:p-6 lg:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Badge className="self-center bg-white/10 px-3 py-1.5 text-[11px] font-semibold text-white sm:text-xs lg:self-auto">
            processo visivel e facil de entender
          </Badge>
        </div>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="rounded-3xl border border-white/10 bg-white/8 p-4 backdrop-blur sm:p-5">
            ...
            <div className="font-display mt-3 text-xl font-semibold text-white sm:text-2xl">
              Atendimento rapido, natural e profissional.
            </div>
            <div className="mt-3 max-w-xl text-sm leading-6 text-neutral-silver sm:text-base sm:leading-7">
              Em vez de esperar resposta ou preencher um processo pesado, o lead
              entra em uma conversa enxuta que já move a oportunidade para o
              ponto certo.
            </div>
          </div>
